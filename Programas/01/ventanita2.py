...
class App:
    def __init__(self, master):
        ...
        self.master.title("Métodos Kutta")
        self.showKs=0
        self.showGraph=0
        ...
        tk.Label(master, text="dy/dx=").grid(row=4, column=0)
    ...
    def mostrar_dataframe(self):
        ...
        resultsWindow = tk.Toplevel(self.master)
        resultsWindow.title("Tablas Runge-Kutta")
        resultsWindow.resizable(width=False,height=False)
        mainFrame = tk.Frame(resultsWindow)
        # titleFrame = tk.Frame(window,height=1)

        if self.showKs == 1:
            kWindow = tk.Toplevel(resultsWindow)
            kWindow.title("Tablas k")
            kWindow.resizable(width=False,height=False)
            kFrame1 = tk.Frame(kWindow)
        ...
        error = 1/h*((RungeKuttas(initialValue=(x,y),h_value=h,point=point, equation=equation).rk4()[["y"]] )-(RungeKuttas(initialValue=(x,y),h_value=h, point=point, equation=equation).rkmMethod()[["y"]])).rename(columns={'y': 'Error'})
